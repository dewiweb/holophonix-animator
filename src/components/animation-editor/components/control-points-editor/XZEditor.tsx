...
export const XZEditor: React.FC<XZEditorProps> = ({
  controlPoints,
  activePlane,
  zoom,
  panOffset,
  ...
  // Convert 3D position to 2D screen coordinates for XZ plane (X horizontal, Z vertical)
  ...
  const screenToPosition = useCallback((screenX: number, screenY: number, containerWidth: number, containerHeight: number, originalPosition?: Position): Position => {
    ...
    if (originalPosition) {
      return {
        ...
        y: originalPosition.y, // Preserve original Y coordinate
    ...
    return {
      x: (screenX - centerX - panOffset.x) / scale,
      y: 0, // Y is not relevant for XZ plane
      z: -(screenY - centerY - panOffset.y) / scale
    }
  }, [zoom, panOffset])
        ...
        switch (animationType) {
          ...
          case 'circular': {
            ...
            if (plane === 'xz') {
              ...
              // For XZ plane, vary X (horizontal) while keeping Z constant at center
          ...
          default:
            ...
            try {
              switch (animationType) {
                case 'elliptical':
                  if (parameters.center && parameters.radiusX && parameters.radiusZ) {
                    ...
                    points.push({ x: t * 2 - 1, y: 0, z: Math.sin(t * Math.PI * 4) })
                  }
                  break
                ...
                case 'spiral':
                  if (parameters.center) {
                    ...
                    points.push({ x: t * 2 - 1, y: 0, z: t * 2 - 1 })
                  }
                  break
                ...
                case 'pendulum':
                  if (parameters.anchorPoint && parameters.pendulumLength) {
                    ...
                    points.push({ x: Math.sin(t * Math.PI * 2), y: 0, z: -Math.cos(t * Math.PI * 2) })
                  }
                  break
                ...
                case 'spring':
                  if (parameters.restPosition) {
                    ...
                    points.push({
                      ...
                      y: rest.y,
                      z: rest.z + displacement.z * Math.sin(omega * t * 10)
                    })
                  } else {
                    points.push({ x: t * 2 - 1, y: 0, z: Math.sin(t * Math.PI * 6) })
                  }
                  break

                case 'lissajous':
                  if (parameters.amplitudeX && parameters.amplitudeZ && parameters.frequencyRatioA && parameters.frequencyRatioB) {
                    ...
                    points.push({ x: Math.sin(t * Math.PI * 2), y: 0, z: Math.sin(t * Math.PI * 4) })
                  }
                  break
  ...
  const render = useCallback(() => {
    ...
    // Draw grid
    if (showGrid) {
      ctx.strokeStyle = themeColors.planes.xz.grid.secondary
      ctx.lineWidth = 1
      ctx.globalAlpha = 0.8
      ...
      if (showMajorGrid) {
        ...
        ctx.strokeStyle = themeColors.planes.xz.grid.primary
        ctx.lineWidth = 1.5
        ctx.globalAlpha = 0.9
    ...
    ctx.strokeStyle = themeColors.planes.xz.axes.x
    ctx.lineWidth = 3
    ctx.beginPath()
    ...
    ctx.strokeStyle = themeColors.planes.xz.axes.z
    ctx.lineWidth = 3
    ctx.beginPath()
    ...
    // Draw origin point (scales with zoom) - Y axis color (blue)
    const originSize = 8 * zoom
    ctx.fillStyle = themeColors.planes.xz.axes.y // Blue for Y axis
    ...
    if (animationPath.length > 0) {
      ctx.strokeStyle = themeColors.planes.xz.trajectory
      ctx.lineWidth = 2
      ctx.globalAlpha = 0.7
      ctx.setLineDash([5, 5])
    ...
    if (trackPosition) {
      ...
      ctx.fillStyle = themeColors.planes.xz.trajectory
      ctx.strokeStyle = '#ffffff'
      ctx.lineWidth = 2
    ...
    if (isSelecting && (selectionStart.x !== selectionEnd.x || selectionStart.y !== selectionEnd.y)) {
      ...
      ctx.strokeStyle = themeColors.planes.xz.selection
      ctx.lineWidth = 2
      ctx.setLineDash([5, 5])
      ...
      ctx.fillStyle = `${themeColors.planes.xz.selection}1A` // 10% opacity
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    setSelectionEnd({ x, y })
  }, [controlPoints, positionToScreen, onDragStart, getPointSize])
  ...
  useEffect(() => {
    ...
    const handleWheelEvent = (e: WheelEvent) => {
      ...
      const newPanOffset = {
        ...
        y: panOffset.y + (worldPos.z * (newZoom - zoom)) // Use Z coordinate for XZ plane
      }
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      <canvas
        ...
        style={{ backgroundColor: themeColors.planes.xz.background.replace('bg-', '#').replace(' dark:bg-', ''), touchAction: 'none' }}
      />
